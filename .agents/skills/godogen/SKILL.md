...
- **VQA skill** — `Skill(skill="visual-qa")` runs in a forked context with Gemini (unbiased, no knowledge of your code). Use for end-to-end validation, after major changes, or whenever you need a fresh assessment. Pass `--both` for Gemini + Codex when higher confidence is needed.
